      ...
      <nav class="stepper" aria-label="Form steps">
        ...
        >
          <div class="stepper__bubble">
            <svg v-if="i < step" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
        ...
        <template v-if="step === 0">
          ...
          <div class="fields">
            <div class="field">
              ...
              <input v-model="form.from" type="email" placeholder="yourname@company.com"
                     :class="{ error: errors.from }" @input="clearErr('from')" />
              <span v-if="errors.from" class="err-msg">{{ errors.from }}</span>
            </div>
            <div class="field">
              <label class="mono-label">To (Recipient Email) <span class="req">*</span></label>
              <input v-model="form.to" type="email" placeholder="manager@company.com"
                     :class="{ error: errors.to }" @input="clearErr('to')" />
            ...
            <div class="field">
              ...
              <input v-model="form.subject" type="text"
                     ...
                     :class="{ error: errors.subject }" @input="clearErr('subject')" />
          ...
          <div class="fields">
            <div class="field">
              ...
              <input v-model="form.date" type="date"
                     :class="{ error: errors.date }" @change="clearErr('date')" />
            ...
            <div class="field">
              ...
              <input v-model="form.payee" type="text"
                     ...
                     :class="{ error: errors.payee }" @input="clearErr('payee')" />
            ...
            <div class="field">
              ...
              <input v-model="form.department" type="text"
                     ...
                     :class="{ error: errors.department }" @input="clearErr('department')" />
          ...
          <div class="fields">
            <div class="field">
              ...
              <div class="input-prefix">
                ...
                <input v-model="form.amountFigures" type="number" min="0.01" step="0.01"
                       placeholder="0.00" class="mono-input"
                       :class="{ error: errors.amountFigures }" @input="clearErr('amountFigures')" />
              </div>
              <span v-if="errors.amountFigures" class="err-msg">{{ errors.amountFigures }}</span>
            </div>
            <div class="field">
              <label class="mono-label">Amount in Words</label>
              <textarea v-model="form.amountWords" rows="2"
                        placeholder="Auto-generated from the figure above"></textarea>
            ...
            <div class="field">
              ...
              <textarea v-model="form.purpose" rows="4"
                        ...
                        :class="{ error: errors.purpose }" @input="clearErr('purpose')"></textarea>
        ...
        <template v-else-if="step === 3">
          ...
          <div class="fields">
            <div class="field">
              ...
              <input v-model="form.submissionDate" type="date"
                     :class="{ error: errors.submissionDate }" @change="clearErr('submissionDate')" />
            ...
            <div class="field">
              ...
              <ul v-if="form.supportingDocs.length" class="file-list">
                <li v-for="(file, i) in form.supportingDocs" :key="i" class="file-item">
                  ...
                  <button type="button" class="file-remove" @click="removeFile(i)" aria-label="Remove">✕</button>
                </li>
              </ul>
          ...
          <div class="modal-body">
            ...
            <div class="preview-section">
              ...
              <div class="preview-rows">
                <div class="preview-row"><span>From</span><span>{{ form.from || '—' }}</span></div>
                <div class="preview-row"><span>To</span><span>{{ form.to || '—' }}</span></div>
                <div class="preview-row"><span>CC</span><span>{{ form.cc || '—' }}</span></div>
                <div class="preview-row"><span>Subject</span><span>{{ form.subject || '—' }}</span></div>
              </div>
            </div>
            ...
            <div class="preview-section">
              ...
              <div class="preview-rows">
                <div class="preview-row"><span>Date</span><span>{{ form.date || '—' }}</span></div>
                <div class="preview-row"><span>Payee</span><span>{{ form.payee || '—' }}</span></div>
                <div class="preview-row"><span>Department</span><span>{{ form.department || '—' }}</span></div>
              </div>
            </div>
            ...
            <div class="preview-section">
              ...
              <div class="preview-rows">
                ...
                <div class="preview-row"><span>Amount (Words)</span><span>{{ form.amountWords || '—' }}</span></div>
            ...
            <div class="preview-section">
              ...
              <div class="preview-rows">
                ...
                <div class="preview-row">
                  ...
                  <span>{{ form.supportingDocs.length ? form.supportingDocs.map(f => f.name).join(', ') : 'None' }}</span>
                </div>
              </div>
            </div>
...
function currentMonth() { return pad(new Date().getMonth() + 1) }
function monthName(m) {
  return ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][parseInt(m) - 1] ?? m
}
function todayStr() { return new Date().toISOString().split('T')[0] }
function makeSerial() { return String(Math.floor(Math.random() * 900) + 100).padStart(3, '0') }
...
// ─── Computed ─────────────────────────────────────────────────────────────────
const deptSlug = computed(() =>
  form.department.trim().toUpperCase().replace(/\s+/g, '-') || 'DEPT'
)
...
function clearErr(key) { delete errors[key] }
...
function removeFile(i) { form.supportingDocs.splice(i, 1) }
...
function reset() {
  Object.assign(form, {
    from: '', to: '', cc: 'finance@gmail.com', subject: '',
    date: todayStr(), payee: '', department: '',
    amountFigures: '', amountWords: '', purpose: '',
    supportingDocs: [], submissionDate: todayStr(),
  })
